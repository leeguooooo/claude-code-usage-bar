...
def try_original_analysis() -> Optional[Dict[str, Any]]:
    """Try to use original project analysis, handling dependency issues"""
    try:
        # Ensure original project path and Python environment
        original_path = Path("/Users/leo/github.com/Claude-Code-Usage-Monitor/src")
        venv_path = Path("/Users/leo/github.com/Claude-Code-Usage-Monitor/venv")
        
        if not original_path.exists():
            return None
        
        # Add venv's site-packages to path (ensure dependencies like pytz are available)
        if venv_path.exists():
        ...
        # Add original project source code path
        if str(original_path) not in sys.path:
...
def calculate_reset_time() -> str:
    ...
    try:
        # Try to get accurate reset time from original project
        original_path = Path("/Users/leo/github.com/Claude-Code-Usage-Monitor/src")
