...
| `bal $809.97` | **Relay account balance** — only in no-quota mode (third-party relay / API key). Auto-detected: a background probe queries the relay's OpenAI-compatible billing endpoint (`/dashboard/billing/subscription` + `/usage`, the new-api / one-api de-facto standard) using your key, computes `hard_limit − used`, and caches it 5 min. **Shown when the relay exposes that endpoint, silently hidden when it doesn't** — zero config. Disable entirely with `cs config set show_balance false`. |
...
| `show_balance` | bool, default `true` | In no-quota mode only, append `bal $X.XX` — your relay account balance. A detached helper probes the relay's OpenAI-compatible billing endpoint (`/dashboard/billing/subscription` + `/usage`) with your key, computes `hard_limit − used`, and caches it (5 min; a relay that doesn't support it is remembered as unsupported for 1 h). Default on but **fully auto**: it only appears if the relay actually answers, so subscribers and unsupported relays see nothing. Set `false` to never probe. |
