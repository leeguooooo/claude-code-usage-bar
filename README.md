...
| `mode_gradient` | bool, default `true` | Tint the mode line with a **static gradient keyed to the effort tier** — a monotonic grey→blue→purple ladder matching Claude Code's own Faster→Smarter slider (low/auto grey, medium/high blue, xhigh indigo, max violet, **ultracode** vivid purple), so the level reads as an ordered ladder at a glance. Static, not animated (an external statusLine refreshes at ≤1 Hz, so motion only flickers). `cs config set mode_gradient false` → plain per-tier text colours. |
