...
| `⧉ <worktree> (3) ⤷ <project> ⎇ <branch>●↑2↓1 · +182 -47 · ⏱ 12m` | Second-line identity + session line. Project comes from Claude Code's `workspace.repo.name` (cwd-basename fallback); branch reads `.git/HEAD` directly; in a linked git worktree the line *opens* with `⧉ <worktree>` in the theme's own worktree hue (violet in most themes) so a non-main checkout is never scanned past — the name is read from `.git`'s `gitdir:` path, so it shows for worktrees Claude Code didn't create too, with a redundant repo prefix trimmed (`repo-wt-x` → `wt-x`) and collapsed to a bare `⧉ worktree` when it would only repeat the branch. In the main checkout the marker becomes a dimmed `⌂ (3)` — same count, saying you're in the main tree with three parallel checkouts around; a repo with no worktrees says `⌂ (0)` rather than going quiet, because an absent marker and a zero count are indistinguishable. The `(3)` is how many linked worktrees this repo has in total — read from `.git/worktrees/`, with prunable ghosts (entry still there, checkout deleted) excluded so the number is trustworthy. Recolor it with `cs config set color_worktree "#bd93f9"`; the `●` dirty marker is refreshed by a background helper, cached 5 s. Enabled by default — turn off with `cs config set show_project_branch false`. `+added -removed` session lines (`show_lines`, +green/−red) also show by default. Opt-in extras live here too: `↑2↓1` commits ahead/behind upstream (`show_ahead_behind`, reuses the dirty-state `git status` — no extra spawn) and session `⏱` duration (`show_duration`). |
